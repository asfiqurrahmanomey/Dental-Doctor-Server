...
function sendBookingEmail(booking) {
    ...
    transporter.sendMail({
        ...
        <div>
            ...
            <p>Thanks from Doctor Service</p>
        </div>
        `, // html body
    }, function (error, info) {
        if (error) {
...
async function run() {
    try {
        // * Appointment Time * //
        const appointmentOptionCollection = client.db('doctors-portal').collection('appointment-Time');
        // * Booking Collection * //
        ...
        /* System to marge data */
        app.get('/appointment-Time', async (req, res) => {
            const date = req.query.date;
            const query = {};
            const options = await appointmentOptionCollection.find(query).toArray();
            // * Get the booking date to provide date * //
        ...
        // * API V2 * //
        app.get('/v2/appointment-Time', async (req, res) => {
            const date = req.query.date;
        ...
        app.post('/bookings', async (req, res) => {
            ...
            console.log(booking);
            // * Search appointment on present date * //
            const query = {
                appointmentDate: booking.appointmentDate,
                email: booking.email,
                treatment: booking.treatment
            }
            // * Appointment on Present Date * //
            const existingDateCount = await bookingsCollection.find(query).toArray();
            if (existingDateCount.length) {
                const message = `You Have a Booking Today on ${booking.appointmentDate}`
        ...
        // // * temporary Update price field insert * //
...
app.listen(port, () => console.log(`Doctors Running on Port ${port}`))